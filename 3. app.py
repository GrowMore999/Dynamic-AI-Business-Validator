...
# --- 3. DYNAMIC DATA ---
CURRENCY_MAP = {"USA": "$", "Europe": "€", "UK": "£", "India": "₹", "Japan": "¥", "UAE": "AED"}
INDUSTRY_TIPS = {
...
def create_pdf(desc, budget, score, report, currency_symbol, budget_verdict):
    ...
    if currency_symbol == "₹": curr_text = "Rs."
    elif currency_symbol == "€": curr_text = "EUR"
...
def sanity_check(desc, budget, raw_score):
    ...
    if budget > limit_high: return max(raw_score - 80, 5.0), "OVERCAPITALIZED", f"⚠️ EXCESSIVE: {budget:,} is inefficient."
    if budget < limit_low: return max(raw_score - 50, 5.0), None, f"⚠️ UNDERFUNDED: {budget:,} is too low."
    return raw_score, None, f"✅ HEALTHY: Budget is standard."
...
with c1:
    ...
    st.markdown(f"**Analytics Dashboard • {industry} • {country}**")
with c2:
...
if run_btn and desc:
    if model:
        with st.spinner("Processing..."):
            ...
            usd_budget = budget * 0.012 if currency == "₹" else budget
        ...
        with g3:
            ...
            <div class="bento-card" style="text-align:center;">
                ...
                <a href="data:application/octet-stream;base64,{b64}" download="VentureReport.pdf" style="text-decoration:none;">
                    <div style="background: linear-gradient(90deg, {risk_color}, #2f3542); padding:8px; border-radius:6px; color:white; font-size:0.8rem; font-weight:bold; margin-top: 10px;">📥 PDF</div>
                </a>
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.markdown('<div class="bento-card"><h3>🔥 STRENGTHS</h3><ul class="pro-list">', unsafe_allow_html=True)
            for p in report['pros']: st.markdown(f"<li>{p}</li>", unsafe_allow_html=True)
            st.markdown('</ul></div>', unsafe_allow_html=True)
        with c2:
            st.markdown('<div class="bento-card"><h3>🔻 RISKS</h3><ul class="con-list">', unsafe_allow_html=True)
            for c in report['cons']: st.markdown(f"<li>{c}</li>", unsafe_allow_html=True)
            st.markdown('</ul></div>', unsafe_allow_html=True)

        d1, d2 = st.columns(2)
        with d1:
            st.markdown(f'<div class="bento-card"><h3>🏢 COMPETITORS ({country})</h3><ul>', unsafe_allow_html=True)
            for c in report['competitors']: st.markdown(f"<li>{c}</li>", unsafe_allow_html=True)
            st.markdown('</ul></div>', unsafe_allow_html=True)
        with d2:
            st.markdown('<div class="bento-card"><h3>🚀 ROADMAP</h3><ul>', unsafe_allow_html=True)
